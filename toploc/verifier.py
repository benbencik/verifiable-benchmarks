...
class Verifier:
    ...
    def verify_proof(self, proof_filename):
        ...
        samples_tensor = torch.tensor(samples_input_list, dtype=samples_tensor_dtype)
        
        print("Recomputing activations using the loaded model and input samples...")
        model.eval() # Ensure model is in evaluation mode
        with torch.no_grad():
            recomputed_output, recomputed_activations_list = model(samples_tensor)
